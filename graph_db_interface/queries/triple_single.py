...
def triple_exists(
    ...
    named_graph = named_graph or self.named_graph
    ...
    if result is None or result["boolean"] is False:
        self.logger.debug(
            f"Unable to find triple ({utils.triple_to_string(triple)}), named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
        )
        return False

    self.logger.debug(
        f"Found triple ({utils.triple_to_string(triple)}), named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
    )
    return True
...
def triple_add(
    ...
    if not result:
        self.logger.warning(
            f"Failed to insert triple: ({utils.triple_to_string(triple)}) named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
        )
        return False

    self.logger.debug(
        f"Successfully inserted triple: ({utils.triple_to_string(triple)}) named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
    )
...
def triple_delete(
    ...
    if check_exist:
        if not self.triple_exists(triple):
    ...
    if not result:
        self.logger.warning(
            f"Failed to delete triple: ({utils.triple_to_string(triple)}), named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
        )
        return False

    self.logger.debug(
        f"Successfully deleted triple: ({utils.triple_to_string(triple)}), named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
    )
    return True
...
def triple_update(
    ...
    new_sub: Optional[IRI] = None,
    new_pred: Optional[IRI] = None,
    new_obj: Optional[IRI] = None,
    check_exist: Optional[bool] = True,
    named_graph: Optional[IRI] = None,
) -> bool:
    """
    ...
    Args:
        ...
        new_sub (Optional[IRI]): Replacement subject.
        new_pred (Optional[IRI]): Replacement predicate.
        new_obj (Optional[IRI]): Replacement object.
    ...
    if check_exist:
        if not self.triple_exists(old_triple):
            self.logger.warning(
    ...
    if not result:
        self.logger.warning(
            f"Failed to update triple to: ({utils.triple_to_string(update_triple)}), named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
        )
        return False

    self.logger.debug(
        f"Successfully updated triple to: ({utils.triple_to_string(update_triple)}), named_graph: {named_graph or self.named_graph or "default"}, repository: {self._repository}"
    )
    return True
